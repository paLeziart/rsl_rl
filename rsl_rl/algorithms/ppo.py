...
from rsl_rl.modules import ActorCritic, ActorCriticCNN, ActorCriticRecurrent, ActorCriticTeacherStudent
...
class PPOCat(PPO):
    ...
    policy: ActorCritic | ActorCriticRecurrent | ActorCriticCNN | ActorCriticTeacherStudent
    """The actor critic module."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        super().__init__(policy, storage, learning_rate=learning_rate, **kwargs)
    ...
    def compute_returns(self, obs: TensorDict) -> None:
        ...
        for step in reversed(range(st.num_transitions_per_env)):
            ...
            # 1 if we are not in a terminal state, 0 otherwise
            next_is_not_terminal = 1.0 - st.dones[step].float()
            next_is_not_terminal *= 1.0 - st.cstr_dones[step].float()  # <-- New line for CaT
            # TD error: r_t + gamma * V(s_{t+1}) - V(s_t)
            delta = st.rewards[step] + next_is_not_terminal * self.gamma * next_values - st.values[step]
            # Advantage: A(s_t, a_t) = delta_t + gamma * lambda * A(s_{t+1}, a_{t+1})
            advantage = delta + next_is_not_terminal * self.gamma * self.lam * advantage
...
class PPOCatTeacherStudent(PPOCat):
    def act(self, obs: TensorDict, switch: torch.Tensor) -> torch.Tensor:
        if self.policy.is_recurrent:
            self.transition.hidden_states = self.policy.get_hidden_states()
        # Compute the actions and values
        self.transition.actions = self.policy.act(obs, switch=switch).detach()
        self.transition.values = self.policy.evaluate(obs).detach()
        self.transition.actions_log_prob = self.policy.get_actions_log_prob(self.transition.actions).detach()
        self.transition.action_mean = self.policy.action_mean.detach()
        self.transition.action_sigma = self.policy.action_std.detach()
    ...
    def process_env_step(
        ...
        # Update the normalizers
        self.policy.update_normalization(obs, switch=extras["switch"])
        if self.rnd:
            self.rnd.update_normalization(obs)
    ...
    def update(self) -> dict[str, float]:
        ...
        for (
            ...
            # Recompute actions log prob and entropy for current batch of transitions
            # Note: We need to do this because we updated the policy with the new parameters
            self.policy.act(obs_batch, masks=masks_batch, hidden_state=hidden_states_batch[0], switch=switch_batch)
            actions_log_prob_batch = self.policy.get_actions_log_prob(actions_batch)
            value_batch = self.policy.evaluate(obs_batch, masks=masks_batch, hidden_state=hidden_states_batch[1])
            # Note: We only keep the entropy of the first augmentation (the original one)
            mu_batch = self.policy.action_mean[:original_batch_size]
            ...
            if self.symmetry:
                ...
                if not self.symmetry["use_data_augmentation"]:
                    ...
                    switch_batch = switch_batch.repeat(num_aug, 1)

                # Actions predicted by the actor for symmetrically-augmented observations
                mean_actions_batch = self.policy.act_inference(obs_batch.detach().clone(),
                                                               switch=switch_batch.detach().clone())
